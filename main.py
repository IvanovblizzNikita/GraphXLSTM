...
from model.molecule_graphxLSTM_optimised import (
    SparseGraphGNN_mLSTM,
    dict_to_cpu,
)
...
def train_for_one_epoch(epoch: int, config: MoleculeConfig, network: SparseGraphGNN_mLSTM, network_weights: dict,
...
def evaluate(eval_type: str, config: MoleculeConfig, network: SparseGraphGNN_mLSTM, objective_evaluator: MoleculeObjectiveEvaluator):
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--objective',
        type=str,
        help="Objective type (e.g. ranolazine_mpo, osimertinib_mpo, ...)"
    )
    ...
    #  Внутри процесса одна GPU она cuda:0
    config.CUDA_VISIBLE_DEVICES = "0"
    config.training_device = "cuda:0"
    config.objective_gnn_device = "cuda:0"

    #  ВОТ СЮДА вставляем развод файлов
    config.gumbeldore_config["destination_path"] = \
        f"./data/generated_molecules_gpu{args.gpu}.pickle"
    ...
    print(f"Objective type           : {config.objective_type}")
    print(f"Num predictor workers    : {config.num_predictor_workers}")
    print(f"Objective GNN device     : {config.objective_gnn_device}")
    print(f"Training device (policy) : {config.training_device}")
    print("=" * 80)
    ...
    network = SparseGraphGNN_mLSTM(
        config=config,
        d_model=config.gnn_d_model,
        num_gnn_layers=config.num_gnn_layers,
        gnn_dropout=config.gnn_dropout,
        gnn_use_jk=config.use_jumping_knowledge,
        num_lstm_layers=config.num_mlstm_layers,
        num_lstm_heads=config.mlstm_num_heads,
        mode="inference",
    )
    ...
    if config.num_epochs == 0:
        ...
        checkpoint = torch.load(
            os.path.join(config.results_path, "best_model.pt"),
            weights_only=False
        )
        network.load_state_dict(checkpoint["model_weights"])
