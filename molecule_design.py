...
class MoleculeDesign(BaseTrajectory):
    ...
    def list_to_batch(molecules: List['MoleculeDesign'], device: torch.device = None,
        ...
        * "additive_padding_attn_mask": torch.FloatTensor of shape (batch_size, <max num atoms>, <max num atoms>), which
        ...
        raw_max_num_atoms = max(num_atoms)

        max_num_atoms = 1 << (raw_max_num_atoms - 1).bit_length()
        ...
        if include_feasibility_masks:
            ...
            num_possible_atom_types = len(molecules[0].vocabulary_atom_idcs)

            num_actions_per_level_and_mol = [
            [num_possible_atom_types + max_num_atoms] * len(molecules),  # lvl 0
            [max_num_atoms - 1] * len(molecules),                             # lvl 1
            [molecules[0].maximum_bond_order] * len(molecules),               # lvl 2
            ]                       
            ...
            for lvl, num_actions_per_mol in enumerate(num_actions_per_level_and_mol):
                ...
                for i, mol in enumerate(molecules):
                    if mol.current_action_level == lvl:
                        ...
                        # 1. Обрезаем, если маска длиннее action space
                        mask = mask[:num_actions_per_mol[i]]

                        # 2. Паддим до общего размера
                        if mask.shape[0] < max_num_actions:
                            mask = np.pad(
                                mask,
                                [(0, max_num_actions - mask.shape[0])],
                                mode="constant",
                                constant_values=1
                            )
                    else:
                ...
                feasibility_masks_per_level.append(
                    torch.from_numpy(np.stack(masks)).bool().to(device)
                )
