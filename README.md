# GraphXLSTM: xLSTM-based Molecular Graph Generation

GraphXLSTM is an extension of the GraphXForm framework for autoregressive molecular graph generation.

The project replaces the original Transformer-based sequence model of GraphXForm with an xLSTM-based policy network and introduces a reproducible Docker-based training environment with automatic dependency patching.

This repository was developed as part of a Master's thesis: *A Comparative Study of Transformer and xLSTM Policy Networks for Molecular Generation*.


## Relation to GraphXForm

This project is based on the original GraphXForm repository:

https://github.com/grimmlab/graphxform

The overall molecular generation workflow and training pipeline follow the original implementation, while the policy network architecture and training environment were substantially modified.


## Main Modifications

Compared to the original GraphXForm implementation, the following changes were introduced:

- Integration of xLSTM-based policy networks into the GraphXForm workflow
- Replacement of the Transformer sequence backbone with mLSTM/xLSTM layers
- Updated configuration system for xLSTM hyperparameters
- Optimized batching and padding logic for xLSTM execution
- Dockerized reproducible environment
- Automatic dependency patching for:
  - xLSTM backend compatibility
  - GuacaMol compatibility with recent SciPy versions
...
The recommended way to run GraphXLSTM is via Docker, as the project depends on CUDA-compatible packages and patched third-party libraries.

Build the Docker image:

```bash
docker build --no-cache -t graphxlstm .
```
...
docker run --gpus all -it \
  --name graphxlstm_container \
  -v $(pwd):/workspace \
  graphxlstm bash
```

The Docker environment installs all Python dependencies from `requirements.txt`, PyTorch Geometric dependencies, xLSTM, and mLSTM kernels.

During image creation, compatibility patches are automatically applied using:

```text
patch_dependencies.py
```

These patches include:

- xLSTM backend compatibility fixes
- GuacaMol compatibility fixes for recent SciPy versions

### Dataset Directories

For pretraining, place the ChEMBL dataset under:

```text
data/chembl/
```

Preprocessed trajectory datasets will be stored under:

```text
data/chembl/pretrain_sequences/
```

Training checkpoints and generated results are saved automatically under:

```text
results/
```

Training checkpoints and generated results will be stored under `results/`.

### GuacaMol

For the drug design tasks, [guacamol](https://github.com/BenevolentAI/guacamol/tree/master) is required.

Compatibility issues related to deprecated `scipy.histogram` imports are automatically patched during Docker image creation via `patch_dependencies.py`.
...
Pretrained GraphXLSTM checkpoints are available on Hugging Face:

https://huggingface.co/IvanovblizzNikita/GraphXLSTM
...
Finetuning the model is performed by running `$ python main.py --gpu`. We note that during finetuning, all layers excpet the last
...
- `gnn_d_model`: [int] Hidden dimensionality of the graph encoder. Defaults to `512`.
- `num_gnn_layers`: [int] Number of graph neural network layers. Defaults to `3`.
- `gnn_dropout`: [float] Dropout used in the graph encoder. Defaults to `0.0`.
- `use_jumping_knowledge`: [bool] Whether Jumping Knowledge aggregation is used in the graph encoder. Defaults to `True`.

##### Sequence Model (xLSTM)

- `num_mlstm_layers`: [int] Number of mLSTM/xLSTM layers in the sequence backbone. Defaults to `10`.
- `mlstm_num_heads`: [int] Number of heads used inside the xLSTM blocks. Defaults to `16`.
- `context_length`: [int] Maximum supported sequence length for the xLSTM backbone. Must be greater than or equal to the maximum molecule length. Defaults to `512`.

##### Compatibility Parameters

- `latent_dimension`: [int] Compatibility parameter retained for logging and legacy utilities. Internally mapped to `gnn_d_model`.
...
- [GraphXForm](https://github.com/grimmlab/graphxform)
